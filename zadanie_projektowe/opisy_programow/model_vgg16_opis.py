...
from torchvision.models import vgg16, VGG16_Weights #zaimportowanie gotowego modelu VGG16 z biblioteki PyTorch oraz jego domyślne (wytrenowane) wagi
...
def get_vgg16(num_classes=39, pretrained=True):
    if pretrained: #chcąc korzystać z przetrenowanego modelu, następuje pobranie jego domyślnych wag
        ...
        model = vgg16(weights=weights) #utworzenie modelu z odpowiednimi wagami
    else:
        model = vgg16(weights=None) #dla wartości pretrained=False model tworzony jest bez domyślnych wag, z losowymi wartosciami
